...
# for song_id in [3, 4]:
    ...
    file = melody_dir + '/%d_vamp_mtg-melodia_melodia_melody.csv'%(song_id)
    ...
    print("--- Melody smoothing: %.2f seconds ---" % (time.time() - start_time))
    ...
    start_time_in_songs = 30
    ind_frame = 1
    for t in xrange(start_time_in_songs,90,1):
        deb = (t -1 )/ 2.0
        fin = (t + 1) / 2.0
        ...
        mu = np.mean(frame[:,1], axis=0)
        sig = np.std(frame[:,1], axis=0)
        # print deb, fin, a
        # print frame[:,0]
        # print deb, a, b
        slopes.append(a)
        mean_std.append([mu, sig])
        # print mu, sig
        # if(len(slopes)>59):
        # break
    ...
    print("--- Slope calculations (lin reg): %.2f seconds ---" % (time.time() - start_time))
    ...
    data['%d'%song_id] = dict()
    data['%d'%song_id]['melody_slope'] = slopes_f
    data['%d'%song_id]['melody_mean_std'] = mean_std
...
# for cle, val in data.iteritems():
#     print 'song_id: %s'%cle
#     for cl, va in data[cle].iteritems():
#         print cl, va.shape
